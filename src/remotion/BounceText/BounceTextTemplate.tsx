...
export const bounceTextSchema = z.object({
    ...
    physicsType: z.enum(['gravity', 'elastic', 'spring', 'rubber', 'jello']).default('gravity'),
...
export const BounceTextTemplate: React.FC<BounceTextProps> = ({
    ...
    const getPhysicsAnimation = () => {
        ...
        switch (physicsType) {
            case 'gravity':
                ...
                const bounceDecay = Math.pow(damping, Math.floor(time / (duration / bounceCount)));
                ...
                if (bounceProgress < 0.5) {
                    // Falling
                    gravityY = -bounceIntensity * bounceDecay * (1 - 4 * bounceProgress * bounceProgress);
                } else {
                    // Rising
                    const riseProgress = (bounceProgress - 0.5) * 2;
                    gravityY = -bounceIntensity * bounceDecay * (1 - 4 * riseProgress * riseProgress);
                }
            ...
            case 'elastic':
                const elasticProgress = Math.sin(time * Math.PI * 2 * bounceCount) *
                    Math.exp(-time * 3 * (1 - elasticity)) * bounceIntensity;
            ...
            case 'spring':
                const springForce = Math.sin(time * Math.PI * 4) *
                    Math.exp(-time * 2 * damping) * bounceIntensity;
            ...
            case 'rubber':
                const rubberBounce = Math.sin(time * Math.PI * 3) *
                    Math.exp(-time * 1.5) * bounceIntensity;
                const rubberStretch = Math.cos(time * Math.PI * 6) *
                    Math.exp(-time * 2) * 0.2;
            ...
            case 'jello':
                ...
                const jelloX = Math.sin(time * Math.PI * jelloFreq) * jelloDecay * bounceIntensity * 0.5;
                const jelloY = Math.cos(time * Math.PI * jelloFreq * 0.7) * jelloDecay * bounceIntensity;
    ...
    const generateParticleExplosion = () => {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            if (particleTime > 0) {
                ...
                const particleY = Math.sin(angle) * velocity * particleTime -
                ...
                particles.push(
                    <div
                        ...
                        style={{
                            ...
                            width: '4px',
                            height: '4px',
                            backgroundColor: colorShift ? `hsl(${(frame + i * 30) % 360}, 70%, 60%)` : color,
            ...
            trails.push(
                <div
                    ...
                    style={{
                        ...
                        color: colorShift ? `hsl(${(frame + i * 20) % 360}, 50%, 50%)` : color,
            ...
            >
                {characters.map((char, index) => {
                    ...
                    const charPhysics = (() => {
                        ...
                        switch (physicsType) {
                            case 'gravity':
                                const charBounceDecay = Math.pow(damping, Math.floor(charTime / (duration / bounceCount)));
                                const charCurrentBounceTime = charTime % (duration / bounceCount);
                                const charBounceProgress = charCurrentBounceTime / (duration / bounceCount);

                                let charGravityY = 0;
                                if (charBounceProgress < 0.5) {
                                    charGravityY = -bounceIntensity * charBounceDecay * (1 - 4 * charBounceProgress * charBounceProgress);
                                } else {
                                    const charRiseProgress = (charBounceProgress - 0.5) * 2;
                                    charGravityY = -bounceIntensity * charBounceDecay * (1 - 4 * charRiseProgress * charRiseProgress);
                                }
                            ...
                            case 'jello':
                                ...
                                const charJelloY = Math.cos(charTime * Math.PI * charJelloFreq * 0.7 + index * 0.5) *
                                    charJelloDecay * bounceIntensity;
                    ...
                    return (
                        <span
                            ...
                            style={{
                                ...
                                color: colorShift ? `hsl(${(frame + index * 30) % 360}, 70%, 60%)` : color,
                                opacity: fadeIn,
                                transform: `translateY(${charPhysics.y}px) rotate(${charPhysics.rotation}deg) scale(${charPhysics.scale})`,
                                textShadow: glowEffect ? `0 0 ${10 + Math.sin(frame / 10 + index) * 5}px ${color}` : '2px 2px 4px rgba(0,0,0,0.3)',
    ...
    return (
        <div style={{
            ...
            >
                {physicsType === 'jello' || text.length > 1 ? renderBouncingText() : (
                    <h1
                        style={{
                            ...
                            textAlign: 'center',
                            margin: 0,
                            textShadow: glowEffect ? `0 0 ${15 + Math.sin(frame / 8) * 10}px ${color}` : '2px 2px 4px rgba(0,0,0,0.3)',
                        }}
                    >
                        {text}
                    </h1>
