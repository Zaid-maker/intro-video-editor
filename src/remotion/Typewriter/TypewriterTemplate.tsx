...
export const typewriterSchema = z.object({
    ...
    typewriterStyle: z.enum(['classic', 'modern', 'glitch', 'vintage']).default('classic'),
    cursorStyle: z.enum(['line', 'block', 'underscore', 'animated']).default('line'),
    ...
    backgroundEffect: z.enum(['none', 'paper', 'terminal', 'matrix']).default('none'),
});
...
export const TypewriterTemplate: React.FC<TypewriterProps> = ({
    ...
    const effectiveFrame = Math.max(0, adjustedFrame - (totalDelay * frame / (text.length * framesPerChar)));
    const charsToShow = Math.min(text.length, Math.floor(effectiveFrame / framesPerChar));
    ...
    if (charsToShow > 0 && charsToShow <= text.length) {
        ...
        if (frame >= popStart && frame <= charAppearFrame) {
            const popIntensity = typewriterStyle === 'modern' ? 1.2 : typewriterStyle === 'glitch' ? 1.6 : 1.4;
    ...
    const getTypewriterStyles = () => {
        ...
        switch (typewriterStyle) {
            ...
            case 'glitch':
                const glitchEffect = Math.random() > 0.95 ? `${Math.random() * 2 - 1}px ${Math.random() * 2 - 1}px 0 rgba(255,0,0,0.5)` : 'none';
                return {
                    ...baseStyles,
                    fontFamily: 'Courier New, monospace',
                    textShadow: glitchEffect,
                    transform: Math.random() > 0.98 ? `translateX(${Math.random() * 4 - 2}px)` : 'none',
                };
            case 'vintage':
                return {
                    ...baseStyles,
    ...
    const getBackgroundStyle = () => {
        const baseStyle = { flex: 1, backgroundColor: bgColor, width, height, padding: 50 };
