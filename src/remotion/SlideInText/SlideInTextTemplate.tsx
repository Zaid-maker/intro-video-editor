...
export const SlideInTextTemplate: React.FC<SlideInTextProps> = ({
    ...
    const getSlideTransform = () => {
        ...
        if (bounce && progress > 0.8) {
            ...
            const bounceOffset = Math.sin(bounceProgress * Math.PI * 4) * 20 * (1 - bounceProgress);
    ...
    const generateParticleTrail = () => {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            const particleX = interpolate(
                ...
                [direction === 'left' ? -100 : direction === 'right' ? width + 100 : Math.random() * width, Math.random() * width],
                { extrapolateRight: 'clamp' }
            );
            ...
            const particleY = interpolate(
                frame,
                [particleDelay, particleDelay + 60],
                [direction === 'top' ? -100 : direction === 'bottom' ? height + 100 : Math.random() * height, Math.random() * height],
                { extrapolateRight: 'clamp' }
            );
    ...
    const renderText = () => {
        ...
        const baseStyles = {
            ...
            textShadow: glowEffect ? `0 0 ${10 + Math.sin(frame / 15) * 10}px ${color}` : '2px 2px 4px rgba(0,0,0,0.3)',
        };
        ...
        switch (slideType) {
            ...
            case 'cascade':
                return (
                    <h1 style={{...baseStyles, display: 'flex', justifyContent: 'center', flexWrap: 'wrap'}}>
                        {characters.map((char, index) => {
                            const charDelay = (index / characters.length) * slideDuration * 0.5;
                            ...
                            const charTransform = direction === 'left' ? 
                            ...
                            return (
                                <span
                                    ...
                                    style={{
                                        ...
                                        opacity: charOpacity,
                                        color: colorShift ? `hsl(${(frame + index * 20) % 360}, 70%, 60%)` : color,
                                    }}
                                >
                                    {char === ' ' ? '\u00A0' : char}
                                </span>
                ...
                return (
                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100%' }}>
                        {words.map((word, wordIndex) => {
                            const wordDelay = (wordIndex / words.length) * slideDuration * 0.3;
                            ...
                            const parallaxOffset = (wordIndex - words.length / 2) * 100 * (1 - wordProgress);
                                ...
                                <span key={wordIndex} style={{ margin: '0 10px' }}>
                                    <span
                                        style={{
                                            ...
                                            color: colorShift ? `hsl(${(frame + wordIndex * 60) % 360}, 70%, 60%)` : color,
                                        }}
                                    >
                                        {word}
                                    </span>
