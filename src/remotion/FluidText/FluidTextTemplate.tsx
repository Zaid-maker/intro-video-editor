    ...
    const getFluidTransform = (index: number) => {
        ...
        switch (fluidType) {
            ...
            case 'liquid':
                ...
                const liquidX = Math.cos(baseTime * 0.5 + charOffset * 0.3) * (waveIntensity * 0.3);
                const liquidScale = 1 + Math.sin(baseTime * 0.7 + charOffset * 0.2) * 0.1;
            ...
            case 'ripple':
                ...
                const ripple = Math.sin(baseTime * 2 - rippleDelay) * waveIntensity * Math.exp(-rippleDistance * 0.1);
                return `translateY(${ripple}px) scale(${1 + ripple * 0.01})`;
                
            case 'flow':
                const flowY = Math.sin(baseTime + charOffset * 0.8) * waveIntensity;
                const flowX = Math.sin(baseTime * 0.3 + charOffset * 0.5) * (waveIntensity * 0.5);
