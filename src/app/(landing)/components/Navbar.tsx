	...
	return (
		<header className="fixed top-0 left-0 right-0 z-50 flex bg-[#0C0C0E] justify-between items-center py-4 px-6 tracking-wide border-b border-white/10">
			<div className="flex items-center space-x-4">
				<Link href={"/dashboard"}>
					<Image height={24} width={25} src={IMAGES.logo} alt="Logo" />
				</Link>
				{pathName === "/dashboard" ? (
					<>
						{data?.user && (
							<span className="text-white font-medium lg:text-md text-xs">
									...
									{tooltipVisible && (
										<div className="absolute right-0 cursor-pointer top-12 z-50 bg-white text-black shadow-xl rounded-lg py-2 w-28 border border-gray-200">
											<button
												...
												className="flex items-center px-4 py-2.5 hover:bg-gray-50 transition-colors duration-150 w-full text-left"
											>
